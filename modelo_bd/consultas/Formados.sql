...
WHERE s.atual = 0 AND NOT EXISTS (
	SELECT d1.id
	FROM Disciplina d1
	INNER JOIN Curriculo_Disciplina cd1 ON d1.id = cd1.Disciplina_id
	INNER JOIN Curriculo c1 ON cd1.Curriculo_id = c1.id
	MINUS
	SELECT d2.id
	FROM Disciplina d2
	INNER JOIN Semestre_Disciplina sd2 ON d2.id = sd2.Semestre_id
	INNER JOIN Semestre s2 ON sd2.Semestre_id = s2.id
	WHERE s2.Aluno_Matricula_numero = a.Matricula_Numero
)
ORDER BY a.nome;