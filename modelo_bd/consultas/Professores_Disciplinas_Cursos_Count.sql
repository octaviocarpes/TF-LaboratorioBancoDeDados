SELECT m.numero as matricula, p.nome as professor, p.genero, p.data_nasc, p.email, p.endereco, c.nome as cidade, e.nome as estado, COUNT(DISTINCT d.nome) as disciplinas, COUNT(DISTINCT cs.nome) as cursos
FROM Professor p
...
GROUP BY m.numero, p.nome, p.genero, p.data_nasc, p.email, p.endereco, c.nome, e.nome
ORDER BY professor;